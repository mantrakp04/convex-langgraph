...
`agent.generateText`, `agent.generateObject`, `agent.streamText`,
and `agent.streamObject`. Any agent can generate a message in a thread created
by any other agent.

```ts
const agent = new Agent(components.agent, { chat: model, instructions: "..." });
